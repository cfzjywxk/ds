...
pub struct QueueIsFullError;

#[derive(Debug, Clone, PartialOrd, PartialEq)]
pub struct QueueIsEmptyError;
...
impl<T> FixedQueue<T> {
    ...
    ///
    pub fn push(&self, data: T) -> Result<(), QueueIsFullError> {
    ...
    ///
    pub fn pop(&self) -> Result<T, QueueIsEmptyError> {
...
mod tests {
    ...
    fn test_basic_push_pop() {
        ...
        assert_eq!(queue.push(4).err().unwrap(), QueueIsFullError);
        assert_eq!(queue.pop().unwrap(), 1);
        assert_eq!(queue.pop().unwrap(), 2);
        assert_eq!(queue.pop().unwrap(), 3);
        assert_eq!(queue.pop().err().unwrap(), QueueIsEmptyError);
